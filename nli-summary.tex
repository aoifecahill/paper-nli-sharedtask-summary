...
\begin{itemize}
\item {\bf Closed-Training}:  The first and main task was be the 11 way
classification task using only the TOEFL11 for training, and then using
TOEFL-TEST for the blind evaluation.
\item {\bf Open-Training}: The second task allowed the use of any amount or
type of training data (as is done in \cite{brooke2011native}) excluding the TOEFL11, but still evaluate on
TOEFL-TEST.
\item {\bf Proficiency Level}: The third task  allowed the use of TOEFL11 combined with any other additional data.
\end{itemize}
...
\end{tabular}
\caption{ \label{tab:commonalities}}
\end{table}
