...
%Were we really the first to notice that some characters only appeared in some languages?
...
in the shared task, and introduce codes that will be used throughout this paper.  Sections~\ref{sec-results}
...
of participating teams, along with their team codes, can be
...
\begin{tabular}{|l|l|}
\hline
\textbf{Team Name} & \textbf{Team Code} \\ \hline
Bobicev & BOB  \\ \hline
Chonger & CHO  \\ \hline
CMU-Haifa & HAI  \\ \hline
...
\end{center}
\end{small}
\caption{Participating Teams and Team Codes\label{tab:teams}}
...
%preferable to keep the training data as close in composition to the test data, however,
%the ``close'' result might only be a function of how much public L1 data is available
%for this task.  For example, the TOR submission had to collect data from several different
%sources.
...
%why are we singling TOR out? What about TUE? where did their extra data come from?
...
\shortcite{tetreault-EtAl:2012:PAPERS} showed that one could achieve even higher
...
Syntactic features, which were first evaluated in Wong and Dras \shortcite{wong-dras:2011:EMNLP} and Swanson and Charniak \shortcite{swanson-charniak:2012:ACL2012short} were used
by six teams in the competition, with most using dependency parses in different ways.
Interestingly, while Wong and Dras \shortcite{wong-dras:2011:EMNLP} showed some of the
...
Spelling features were championed in the Koppel et al. \shortcite{koppel2005determining}
